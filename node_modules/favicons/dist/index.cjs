...
class Images {
  ...
  async createPlaneFavicon(sourceset, options, name, raw = false) {
    ...
    const contents = raw ? await pipeline.toColorspace("srgb").raw({ depth: "uchar" }).toBuffer({ resolveWithObject: true }) : await pipeline.png().toBuffer();
...
class AndroidPlatform extends Platform {
  ...
  async createImages(sourceset) {
    ...
    if (this.options.manifestMaskable && typeof this.options.manifestMaskable !== "boolean") {
      ...
      const maskable2 = await Promise.all(Object.entries(ICONS_OPTIONS_MASKABLE).map(([iconName, iconOption]) => images.createFavicon(maskableSourceset, iconName, iconOption)));
  ...
  async shortcutIcons() {
    const images = new Images();
    const icons = await Promise.all(this.options.shortcuts.map(async (shortcut, index) => {
      if (!shortcut.name || !shortcut.url || !shortcut.icon)
        return null;
      const shortcutSourceset = await sourceImages(shortcut.icon);
      return Promise.all(Object.entries(SHORTCUT_ICONS_OPTIONS).map(([shortcutName, option]) => images.createFavicon(shortcutSourceset, `shortcut${index + 1}-${shortcutName}`, option)));
    }));
    return icons.flat();
  }
  manifestFileName() {
    ...
    return this.options.shortcuts.map((shortcut, index) => {
      ...
      return {
        ...
        icons: shortcut.icon ? Object.entries(SHORTCUT_ICONS_OPTIONS).map(([shortcutName, option]) => {
          ...
          return {
            src: relativeTo(basePath, `shortcut${index + 1}-${shortcutName}`),
            sizes: `${width}x${height}`,
            type: "image/png"
          };
        }) : void 0
...
class WindowsPlatform extends Platform {
  ...
  browserConfig() {
    ...
    for (const { name, ...size } of SUPPORTED_TILES) {
      const icon = Object.entries(this.iconOptions).find((icon2) => hasSize(size, icon2[1]));
      if (icon) {
        tile[name] = {
...
function favicons(source, options = {}, next = void 0) {
  if (next) {
